<template>
  <GridSelector
    class="oneshot-race-middle-grid"
...
const emit = defineEmits(["select-middle"])
...
const pathSegments = computed(() => [props.mode.middlePathKey, props.levelName])
const defaultPath = computed(() => ({ keys: pathSegments.value }))
...
const nextStepButton = computed(() => ({
  icon: "fastTravel",
  label: $translate.instant(props.mode.selectVehicleLabelKey),
}))

function onNextStepClick({ activeItem }) {
  if (!activeItem?.showDetails?.middleName) return
  emit("select-middle", props.levelName, activeItem.showDetails.middleName)
}
</script>

<style scoped lang="scss">
.oneshot-race-middle-grid {
  flex: 1 1 auto;
  min-height: 0;
}
</style>
